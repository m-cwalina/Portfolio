    ...
    <Box sx={{ display: 'flex', justifyContent: 'space-around' }}>
      <Box sx={{ width: '50%' }}>
        ...
        <Typography
          ...
          <p>
            This is a PostsSlice which handles pagination, errors, and async API calls. My state has 5 attributes that are used together to handle a Post Object. The first attribute data, is used to hold all posts from the backend. The Posts are always added onto the array while scrolling. The second attribute loading, handles a loading state for initial load and loading more posts onto the posts object. While scrolling, an eventListener triggers a useEffect which fetches more posts and provides an infinite scroll functionality. I have included a morePosts flag set to true to manage when there are no more posts available from the backend. When a object from the backend has an attribute false, I capture the inequality and set the attribute to false on the Post Object. This stops constant API calls when their are no more Posts to fetch.
          </p>
          <p>
